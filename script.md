...
CREATE OR REPLACE TEMP TABLE tmp_inventory_details AS (
  ...
  , location_ids AS (
    SELECT
      ...
      ti.sprid,
      tsi.location_id,
    FROM `wf-gcp-us-ae-sql-data-prod.aad.tbl_item` ti
    INNER JOIN `wf-gcp-us-ae-sql-data-prod.aad.t_stored_item` tsi
  ...
  FROM `wf-gcp-us-ae-sql-data-prod.csn_inventory_trust.tbl_inventory_quantity` iq
  LEFT JOIN `wf-gcp-us-ae-sql-data-prod.elt_wms.tbl_stock_product_owner` o
    ON iq.ownerid = o.sprownerid
  LEFT JOIN `wf-gcp-us-ae-sql-data-prod.csn_order.tbl_supplier` s2
    ON o.sprownerSuID = s2.suid
  JOIN `wf-gcp-us-ae-sql-data-prod.csn_wms.tbl_stock_product` spr
    ON spr.SprID = iq.SprID
  JOIN `wf-gcp-us-ae-sql-data-prod.csn_wms.tbl_wms_supplier` wms
    ON wms.WSWHID = iq.WHID
  LEFT JOIN `wf-gcp-us-ae-sql-data-prod.csn_wms.tbl_join_supplier_stock_product` jssp
...
CREATE OR REPLACE TABLE `wf-gcp-us-ae-ops-prod.csn_junk.tbl_Hazmat_loc_DD`  as
...
CREATE OR REPLACE TEMP TABLE tmp_shipping_details AS (
    ...
    (
        ...
        FROM
            `wf-gcp-us-ae-bulk-prod.aad.t_tran_log` ttl
        INNER JOIN
            `wf-gcp-us-ae-bulk-prod.aad.t_location` loc
        ON
                ttl.wh_id = loc.wh_id
        ...
        INNER JOIN
            `wf-gcp-us-ae-bulk-prod.aad.t_order` tor
        ON
        ...
        INNER JOIN
            `wf-gcp-us-ae-bulk-prod.aad.t_whse` wh
        ON
                ttl.wh_id = wh.wh_id
...
CREATE OR REPLACE TABLE `wf-gcp-us-ae-ops-prod.csn_junk.tbl_Hazmat_shipping_details`  as
( select * from tmp_shipping_details)                                    ------------------- Perm table creation
;


CREATE or REPLACE TABLE `wf-gcp-us-ae-ops-prod.csn_junk.maintable` AS
(  SELECT SAFE_CAST(m.store_order_number as int64) as Store_order_num_int,*
FROM `wf-gcp-us-ae-ops-prod.csn_junk.tbl_Hazmat_shipping_details` m
INNER JOIN (
  SELECT order_number as Ordernumber,SUM(tran_qty) as Total_hazmat
  FROM `wf-gcp-us-ae-ops-prod.csn_junk.tbl_Hazmat_shipping_details`
...
,Haz_Mat_Weightm
,Lithium_Batterym
 FROM `wf-gcp-us-ae-ops-prod.csn_junk.tbl_Hazmat_loc_DD`
  ) lj
...
CREATE OR REPLACE TEMP TABLE tmp_supplier_details AS (  
    ...
    WITH wayfaircatalog AS (  
        ...
        from `wf-gcp-us-ae-sql-data-prod.elt_product.tbl_product` p                            
        INNER JOIN `wf-gcp-us-ae-sql-data-prod.elt_product.tbl_join_product_class` pc
            ON pc.prsku = p.PrSKU                          
        INNER JOIN `wf-gcp-us-ae-sql-data-prod.elt_product.tbl_class` c
            ON c.clid = pc.ClID                            
        INNER JOIN `wf-gcp-us-ae-sql-data-prod.elt_product.tblpl_product_status` ps
            ON ps.prstatus = p.prstatus                                
        LEFT OUTER JOIN `wf-gcp-us-ae-sql-data-prod.elt_product.tbl_product_additional` a
            ON a.prsku = p.PrSKU                                                                                                            
        LEFT OUTER JOIN `wf-gcp-us-ae-sql-data-prod.elt_product.tblpl_discontinued_reason` dr
            ON dr.DrID = a.PrDrID                                                                                          
        LEFT OUTER JOIN `wf-gcp-us-ae-sql-data-prod.elt_product.tblpl_admin_scan_sell_reason` ac
            ON ac.AcsID = a.PrAcsID                                        
        LEFT OUTER JOIN `wf-gcp-us-ae-sql-data-prod.elt_ds_catalog_identifier.vw_product_supplier_manufacturer_part` psmp
            ON p.PrSKU = psmp.PrSKU        
        LEFT OUTER JOIN `wf-gcp-us-ae-sql-data-prod.elt_order.tbl_supplier` s
            ON s.SuID = psmp.SupplierID
        LEFT OUTER JOIN `wf-gcp-us-ae-sql-data-prod.elt_product.tblpl_country`
            ON cyid = prcyid                        
        )
...
select * from wayfaircatalog wc
INNER JOIN  rest hz
    ON  hz.SKU = wc.PrSKU
    AND hz.Suppid = wc.SupplierID
WHERE
   wc.PrSKU IN (SELECT prsku from `wf-gcp-us-ae-ops-prod.csn_whs_reporting.tbl_opra_hazmat_classification`  )    
and  wc.prstatusname = "Live Product"
order by wc.PrSKU  
);


CREATE OR REPLACE TABLE `wf-gcp-us-ae-ops-prod.csn_junk.tbl_Hazmat_supplier_details`  as
...
CREATE OR REPLACE TABLE `wf-gcp-us-ae-ops-prod.csn_junk.tbl_Hazmat_quantity_OH`  as(
  SELECT
    iq.SprID,
    spr.SprPrSKU,
    CONCAT(iq.WHID) AS WHID, -- WHID is now a STRING
